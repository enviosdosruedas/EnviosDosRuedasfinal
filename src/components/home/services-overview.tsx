...
export function ServicesOverview() {
  ...
  // Duplicamos los servicios para el efecto de carrusel infinito
    ...
    <section className="py-16 md:py-20 lg:py-24 px-4 bg-background">
      <div className="container mx-auto">
        ...
        <div className="md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-8">
            <div className="flex space-x-6 overflow-x-auto md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-8 w-full md:contents group">
              <div className="flex w-max animate-h-scroll group-hover:[animation-play-state:paused] md:contents">
                {duplicatedServices.map((service, index) => (
                  <motion.div
                    key={index}
                    className="relative group/card rounded-lg overflow-hidden shadow-lg h-[450px] w-[80vw] sm:w-[60vw] md:w-auto flex-shrink-0"
                    whileHover="hover"
                    initial="initial"
                    variants={{
                      initial: { y: 0 },
                  ...
                  >
                    ...
                    <motion.div
                      className="relative z-10 p-6 flex flex-col h-full justify-end md:group-hover/card:translate-y-0 md:translate-y-[calc(100%-80px)] transition-transform duration-500 ease-in-out"
                    >
                      <h3 className="text-2xl font-bold text-white mb-2 font-heading">{service.title}</h3>
                      
                      <div className="md:opacity-0 group-hover/card:md:opacity-100 transition-opacity duration-300 delay-100">
                        <p
